    ...
    _root_node(nullptr)
{
    ...
    ret->registerModel<IsFreeToObject>("Condition");

    ret->registerModel<IsObjectOnGoal>("Condition");
    ret->registerModel<Place>("Action");
    ret->registerModel<IsFreeToGoal>("Condition");

    ret->registerModel<ClearPathTo>("Action");
    ret->registerModel<DropObject>("Action");
