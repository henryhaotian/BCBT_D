...
BT::TreeNode *getBTObject(QtNodes::FlowScene &scene, QtNodes::Node &node)
{
    ...
    case BT::ISFREETOOBJECT:
    {
        BT::IsFreeToObject* bt_node = new BT::IsFreeToObject(node.nodeDataModel()->get_line_edit().toStdString());
        node.linkBTNode(bt_node);
        return bt_node;
    }
    case BT::PLACE:
    {
        ...
        BT::Place* bt_node = new BT::Place(name, goal);
        node.linkBTNode(bt_node);
        return bt_node;
    }
    case BT::ISOBJECTONGOAL:
    {
        std::string s = node.nodeDataModel()->get_line_edit().toStdString();
        std::size_t found = s.find_last_of(",");
        std::string name = s.substr(0,found);
        std::string goal =  s.substr(found+1);
        BT::IsObjectOnGoal* bt_node = new BT::IsObjectOnGoal(name, goal);
        node.linkBTNode(bt_node);
        return bt_node;
    }
    case BT::ISFREETOGOAL:
    {
        BT::IsFreeToGoal* bt_node = new BT::IsFreeToGoal(node.nodeDataModel()->get_line_edit().toStdString());
        node.linkBTNode(bt_node);
        return bt_node;
    }
    case BT::CLEARPATHTO:
    {
        BT::ClearPathTo* bt_node = new BT::ClearPathTo(node.nodeDataModel()->get_line_edit().toStdString());
        node.linkBTNode(bt_node);
        return bt_node;
    }
    case BT::DROPOBJECT:
    {
        BT::DropObject* bt_node = new BT::DropObject(node.nodeDataModel()->get_line_edit().toStdString());
        node.linkBTNode(bt_node);
        return bt_node;
    }
    case BT::SEQUENCE:
